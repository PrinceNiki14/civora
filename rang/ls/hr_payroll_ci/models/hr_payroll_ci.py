...
class HrPayslip(models.Model):
    ...
    #         if trouver == True :
    #             raise ValidationError(_("L'employé possède déjÃ  un bulletin pour cette période"))
    #         else :
    ...
    def _get_worked_day_lines(self):
        """
        Calcul dynamique des jours et heures travaillés
        Avec gestion par défaut si aucune présence n'est trouvée
        """
        res = []
        self.ensure_one()
        ...
        total_worked_days = len(set(attendance.check_in.date() for attendance in attendances))
        total_worked_hours = sum(
            (attendance.check_out - attendance.check_in).total_seconds() / 3600
            for attendance in attendances
        )

        # Gestion des jours travaillés par défaut
        if total_worked_days == 0:
            total_worked_days = 30  # Valeur par défaut

        # Gestion des heures travaillées par défaut
        if total_worked_hours == 0:
            total_worked_hours = 240  # Valeur par défaut

        # Limitation Ã  240 heures
        if total_worked_hours > 240:
            total_worked_hours = 240

        # Vérification des valeurs dans hr_payroll_structure
        payroll_structure = self.struct_id  # Assurez-vous que struct_id pointe vers hr_payroll_structure

        if payroll_structure:
            # Vérifiez et utilisez les variables si elles ne sont pas nulles
            if payroll_structure.number_days:
                total_worked_days = payroll_structure.number_days
            if payroll_structure.number_hours:
                total_worked_hours = payroll_structure.number_hours

        # Récupération du type d'entrée de travail
        ...
        if work_entry_type:
            attendance_line = {
                ...
                'number_of_days': total_worked_hours / 8,  # Conversion en jours si nécessaire
                'number_of_hours': total_worked_hours,
                'rate': total_worked_days / 30,  # Taux basé sur les jours
                'amount': 0,
            }
    ...
    cumul_brut = fields.Float('Cumul base impÃ´t', compute='_get_rubrique_total')
...
class HrPayslipLine(models.Model):
    ...
    quantity = fields.Float('Quantity', digits=(16, 9))
...
class HrPayslipWorkedDays(models.Model):
    ...
    def _computeRate(self):
        for wk_days in self:
            # Récupération de la structure de paie via le bulletin de paie
            payroll_structure = wk_days.payslip_id.struct_id
            if wk_days.code == "WORK100":
                if payroll_structure and payroll_structure.number_hours:
                    wk_days.rate = wk_days.number_of_hours / payroll_structure.number_hours
                    wk_days.number_of_days = wk_days.number_of_hours / 8
                else:
                    wk_days.rate = wk_days.number_of_hours / 240
                    wk_days.number_of_days = wk_days.number_of_hours / 8
            else:
                wk_days.rate = 0

    rate = fields.Float('Taux', digits=(12, 9), compute='_computeRate')
