...
class HrEmployee(models.Model):
    ...
    def _get_indemnite_licencement(self):
        for emp in self:
            contract_id = emp.env['hr.contract'].search([('employee_id', '=', emp.id), ('state', '=', 'open')])
            if emp.debut_decompte and emp.debut_rupture and not emp.is_retraite:
                ...
                if contract_id.an_anciennete:
                    year = contract_id.an_anciennete
                    if 0 <= year <= 6:
                        amount = (SNMM * 30) / 100
    ...
    def _get_indemnite_deces(self):
        for emp in self:
            ...
            #print(employee)
            contract_id = emp.env['hr.contract'].search([('employee_id', '=', emp.id), ('state', '=', 'open')])
            if contract_id.an_anciennete and emp.is_deces:
                year = contract_id.an_anciennete
            ...
            if contract_id.an_anciennete and not emp.is_deces:
                emp.indemnite_deces = 0
            if not contract_id.an_anciennete and not emp.is_deces:
                emp.indemnite_deces = 0
            if not contract_id.an_anciennete and emp.is_deces:
                emp.indemnite_deces = 0
    ...
    def get_montant_moyen_journalier(self):
        ...
        for emp in self:
            # emp_id = emp.ids[0]
            contract_id = emp.env['hr.contract'].search([('employee_id', '=', emp.id), ('state', '=', 'open')])
            if contract_id.date_start and not emp.date_retour_conge:
                payslip = slip_obj.search(
                    [('employee_id', '=', emp.id), ('date_from', '>=', contract_id.date_start)])
                ...
                if payslip != 0:
                    ...
                    return SMJ
            elif emp.date_retour_conge and contract_id.date_start and not emp.debut_rupture:
    ...
    def _get_jour_conge(self):
        for emp in self:
            ...
            contract_id = emp.env['hr.contract'].search([('employee_id', '=', emp.id), ('state', '=', 'open')])
            if contract_id.date_start and not emp.date_retour_conge:
                payslip = self.env['hr.payslip'].search([('employee_id', '=', emp.id), ('date_from', '>=', contract_id.date_start)])
                worked_days_number = list()
                if payslip:
            ...
            elif emp.date_retour_conge and contract_id.date_start:
    ...
    def _compute_conge_non_exceptionnel(self):
        for rec in self:
            hr_holidays = (rec.env['hr.leave'].search(
                [('employee_id', '=', rec.ids[0]), ('state', '=', 'validate'), ('conge_non_exceptionne', '=', True)])
                           .filtered(lambda r: r.holiday_status_id.code == 'CONG_NON_EXCEPTIONNEL'))
            if hr_holidays:
                days = 0
                for holy in hr_holidays:
                    days += holy.number_of_days
    ...
    def get_montant_by_periode_reference(self):
        ...
        for emp in self:
            ...
            if contract_id:
                if contract_id.date_start and not emp.date_retour_conge:
                    payslip = slip_obj.search([('employee_id', '=', emp.id), ('date_from', '>=', contract_id.date_start)])
                    number = len(payslip)
                    if payslip != 0:
                        for slip in payslip:
                            for line in slip.line_ids:
                    ...
                    print('emp.date_retour_conge', emp.date_retour_conge)
                    payslips = slip_obj.search([('employee_id', '=', emp.id), ('date_from', '>=', emp.date_retour_conge)])
                    #payslip = slip_obj.browse(payslips)
                    #number = len(payslip)
                    if payslips != 0:
    ...
    def _get_indemnite_fin_cdd(self):
        ...
        for emp in self:
            ...
            contract_id = emp.env['hr.contract'].search([('employee_id', '=', emp.id), ('state', '=', 'open')])
            employee = emp.env['hr.employee'].search([('id', '=', emp.id)])
            if contract_id.date_start and contract_id.date_end:
                ...
                if payslip != 0:
                    ...
                    if montant:
                        ...
                        #employee.write({'indemnite_fin_cdd2': indemnite_fin_cdd2})contract_id.date_end:
